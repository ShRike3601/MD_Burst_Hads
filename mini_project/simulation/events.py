class TaskCompleteEvent:
    ...
    def execute(self):
        ...
        # start the next queued task on this VM
        if self.vm.tasks:
            ...
            # only apply work stealing if the scheduler supports slack checks
        ...
        # trigger rescheduling after each completion
...
class HibernationEvent:
    ...
    def execute(self):
        ...
        for task in tasks_to_migrate:
            ...
            if new_vm is not None and new_vm.tasks and new_vm.tasks[0] == task:
                execution_time = task.remaining_time / new_vm.speed
                finish_time = self.time + execution_time

                event = TaskCompleteEvent(
                    finish_time, task, new_vm, self.scheduler.event_engine
                )
        ...
        # reschedule after VM hibernation
